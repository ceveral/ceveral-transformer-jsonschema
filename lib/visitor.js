...
class JSONSchemaVisitor extends ceveral_compiler_1.BaseVisitor {
    parse(expression) {
    ...
    visitPackage(expression) {
        ...
        let records = expression.children.filter(m => m.nodeType == ceveral_compiler_1.Token.Record).map(m => this.visit(m));
        return records;
    }
    visitUserType(expression) {
        ...
        this.optional = false;
        this.required = [];
        let r = this.package.children.find(m => m.nodeType == ceveral_compiler_1.Token.Record && m.name == expression.name);
    ...
    visitImportType(expression) {
        ...
        let r = i.children.find(m => m.nodeType == ceveral_compiler_1.Token.Record && m.name == expression.name);
        let oldOptional = this.optional;
        let oldRequired = this.required;
    ...
    visitMapType(expression) {
        if (expression.key.type !== ceveral_compiler_1.Type.String) {
    ...
    visitAnnotation(expression) {
        ...
        if (expression.name === 'schemaformat') {
            if (formats.indexOf(expression.args) === -1)
                throw new ceveral_compiler_1.ValidationError("schemaformat must be " + formats.join('|'));
        }
        return expression;
    }
...
exports.JSONSchemaVisitor = JSONSchemaVisitor;
